...
class RegisterScreen extends React.Component {
  ...
  rendercuisines=()=>{
    return (
        <View style={{ flexDirection: 'row',alignItems:'center' ,marginTop:1*vh}}>
            {this.state.restaurant.cusines.map((item, index) => {
                return (
                   <RubikLight style={styles.cus}>{item?.name},</RubikLight>
...
renderratings = () => {
  return (
      <View style={{ flexDirection: 'row',alignItems:'center',justifyContent:'space-between',marginTop:1*vh ,width:65*vw}}>
      ...
      markers?.map(location => {
        return (
          <Marker
            ...
            image={icons.marker}>
            <Callout style={{borderRadius:vh}}
            onPress={()=> this.props.navigation.navigate('RestaurantDetails')}>
              <View style={styles.boxview}>
                <View style={{height:10*vh}}>
                <Text style={{ position: "relative", bottom: 75,height:20*vh }}> 
                  <Image source={this.state.restaurant.image}
                ...
                <View style={{  marginTop: vh }}>
                  <GilroyBold style={styles.name}>{this.state.restaurant.name}</GilroyBold>
